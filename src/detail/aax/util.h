...
public:
    ...
    bool push(const ParamChange& change)
    {
        ...
        if (nextWrite - _readIndex.load(std::memory_order_acquire) > _capacity)
            return false;
    ...
    bool pop(ParamChange& outChange)
    {
        ...
        if (read == _writeIndex.load(std::memory_order_acquire))
            return false; // empty
    ...
    void clear()
    {
        _readIndex.store(_writeIndex.load(std::memory_order_relaxed),
