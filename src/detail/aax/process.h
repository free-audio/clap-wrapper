...
struct SAAX_Wrapper_AlgorithmicContext
{
  ...
  int32_t* mNumSamples = nullptr;  ///< Number of samples in each buffer.  Bounded as per \ref AAE_EAudioBufferLengthNative.
  ...
  AAX_IMIDINode* mInputNode = nullptr;  ///< Buffered local MIDI input node. Used for incoming MIDI messages directed to the instrument.
  AAX_IMIDINode* mOutputNode = nullptr;  ///< Buffered local MIDI input node. Used for incoming MIDI messages directed to the instrument.
  AAX_IMIDINode* mGlobalNode = nullptr;  ///< Buffered global MIDI input node. Used for global events like beat updates in metronomes.
  AAX_IMIDINode* mTransportNode = nullptr;  ///< Transport MIDI node.  Used for querying the state of the MIDI transport.
  ...
  float** mMeters = nullptr;  ///< Array of meter taps.  One meter value should be entered per tap for each render call.
