...
static_assert((kMidiOutRingSize & kMidiOutRingMask) == 0,
              "kMidiOutRingSize must be a power of two");
}
...
- (void)setupMIDIOutput
{
    ...
    OSStatus st = MIDISourceCreate(self.midiClient,
                                   (__bridge CFStringRef)sourceName, &src);
    if (st != noErr)
    {
    ...
    self.audioUnit.MIDIOutputEventBlock = ^OSStatus(AUEventSampleTime sampleTime,
                                                    uint8_t cable,
                                                    NSInteger length,
                                                    const uint8_t *data) {
        (void)cable;
    ...
    dispatch_source_set_timer(timer, dispatch_time(DISPATCH_TIME_NOW, 0),
                              1 * NSEC_PER_MSEC, 100 * NSEC_PER_USEC);
    ...
    dispatch_source_set_event_handler(timer, ^{
        ...
        while (head != tail)
        {
            ...
            pkt = MIDIPacketListAdd(&list, sizeof(list), pkt,
                                    mach_absolute_time(), slot.length, slot.data);
    ...
    NSLog(@"[ios-host] MIDI-out: virtual source \"%@\" ready (%lu port(s) advertised)",
          sourceName, (unsigned long)self.audioUnit.MIDIOutputNames.count);
}
...
- (void)handleRouteChange:(NSNotification *)note
{
    ...
    AVAudioSession *session = [AVAudioSession sharedInstance];
    NSLog(@"[ios-host] route change: reason=%lu rate=%.0f buffer=%.3fms",
          (unsigned long)reason, session.sampleRate, session.IOBufferDuration * 1000.0);
