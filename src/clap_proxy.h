...
public:
  ...
  const uint8_t* data() { return _mem.data(); }
  size_t size() { return _mem.size(); }
  void setData(const uint8_t* data, size_t size);
  operator const clap_ostream_t*() { return &_outstream; }
  operator const clap_istream_t*() { return &_instream; }
private:
