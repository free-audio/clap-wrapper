...
static const clap_plugin_descriptor_t s_clap1stDist_desc = {
    CLAP_VERSION_INIT,
    "org.free-audio.clap-macos-embedded",
    "ClapMacosEmbedded",
    "Surge Synth Team",
                               ...
                               clap_plugin_info_as_auv2_t* info)
{
  ...
  strncpy(info->au_type, "aumf", 5);  // use the features to determine the type

  if (strcmp(plugin_id, plugin_id) == 0)
  {
    strncpy(info->au_subt, "bdMF", 5);
    return true;
  }
...
const struct clap_plugin_factory_as_auv2 s_auv2_factory = {"FRaD",        // manu
